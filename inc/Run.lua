...
local url , res = https.request('https://ibotcorp.com/api/GetUser/?id='..SUDO_USER..'&param=all')
if res ~= 200 then
...
if not GetUser.result or not GetUser.ok then
if GetUser.description then
print('\n\27[1;31m￤ '..GetUser.description)
os.exit()
end
print('\n\27[1;31m￤ USERNAME is Incorrect Please Check it!\n￤ لآ يوجد حسـآب بهہ‏‏ذآ آلمـعرف , تآگد مـنهہ‏‏ جيدآ  !')
create_config(Token)
end  
print('\n\27[1;36m￤تم آدخآل مـعرف آلمـطـور بنجآح , سـوف يتم تشـغيل آلسـورس آلآن .\n￤Success Save USERNAME IS_ID: \27[0;32m['..GetUser.result.id..']\n\27[0;39;49m')
boss = Token:match("(%d+)")
redis:set(boss..":VERSION",GetUser.by)
redis:set(boss..":SUDO_ID:",GetUser.result.id)
--redis:set(boss..":DataCenter:",GetUser.result.DataCenter) هذا شلته لان مادري همه شمسوين
redis:set(boss..":UserNameBot:",BOT_User)
redis:set(boss..":NameBot:",BOT_NAME)
redis:hset(boss..'username:'..GetUser.result.id,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
...
local Text = "🙋🏼‍♂️¦ اهلا عزيزي [المطور الاساسي](tg://user?id="..GetUser.result.id..") \n🔖¦ شكرا لاستخدامك سورس الزعيم \n📡¦ أرســل  الان /start\n📛¦ لاضهار الاوامر للمطور  المجهزه بالكيبورد\n\n⚡️"
https.request(Api_Token..'/sendMessage?chat_id='..GetUser.result.id..'&text='..URL.escape(Text)..'&parse_mode=Markdown')
os.execute([[
rm -f ./README.md
rm -rf ./.git
chmod +x ./run
...
function tdcli_update_callback(data)
	...
	download_file('https://raw.githubusercontent.com/ramikong/rami/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/ramikong/rami/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/ramikong/rami/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/ramikong/rami/master/inc/functions.lua','./inc/functions.lua')
